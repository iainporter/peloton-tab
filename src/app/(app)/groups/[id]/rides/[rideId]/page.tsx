...
export default async function RideDetailPage({
  ...
  const riders = await db
    .select({
      ...
      stravaActivityId: rideRiders.stravaActivityId,
    })
    .from(rideRiders)
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {ride.autoDetected && riders.find((r) => r.id === session.user.id)?.stravaActivityId && (
              <a
                href={`https://www.strava.com/activities/${riders.find((r) => r.id === session.user.id)!.stravaActivityId}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-xs font-bold text-[#FC5200] hover:opacity-80"
              >
                View on Strava
